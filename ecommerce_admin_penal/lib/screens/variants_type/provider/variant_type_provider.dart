  ...
  updateVariantType() async {
    try {
      if(variantTypeForUpdate != null){
        Map<String, dynamic> variantTypes = {'name': variantNameCtrl.text,'type': variantTypeCtrl.text};
        final response = await service.updateItem(endpointUrl: 'variantTypes', itemData: variantTypes, itemId: variantTypeForUpdate?.sId ??'');
        if(response.isOk){
          ApiResponse apiResponse = ApiResponse.fromJson(response.body, null);
          if(apiResponse.success == true){
            clearFields();
